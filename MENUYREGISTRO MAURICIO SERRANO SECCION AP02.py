...
Gri_Hoy = datetime.datetime.today()
Lat_Hoy = Gri_Hoy.strftime("%d/%m/%Y")
Fe_Des = "00/00/0000"
Fe_Mod = "00/00/0000"
...
def Registrar():
    with open('BASE_DE_DATOS.txt', 'a') as BD:
        ...
        while True:
            ...
            BD.writelines(f"{Num_B_Pub} {SeriaL} {Marca} {Modelo} {Lat_Hoy} {Fe_Des} {Fe_Mod}\n")
            Limpia()
            Seguir = input("\t\t¿Usted desea registrar los datos de otro equipo? - [SI/NO] ").upper()
            if Seguir == "NO":
                print("\t\t\tUsted ha decidido no seguir registrando datos.\n")
                break
            elif Seguir == "SI":
                print("\t\t\tUsted decidió registrar más equipos.\n")
            else:
                Seguir = input("\t\tIngrese sólo 'SI' o 'NO': ")
            Limpia()
    BD.close()
...
def Menu():
    while True:
        ...
        if ELEC == "1":
            ...
            #Desincorporar()
            print("\t\t\tINGRESÓ EN DESINSCORPORAR UN DATO DEL INVENTARIO\n\n")
        elif ELEC == "5":
